...
def load_results(path, filename, verbose=True):
    ...
    返回:
        加载的结果字典
    """
    # 设置allow_pickle=True以允许加载包含Python对象的数组
...
def get_list_of_experiments(path, only_finished=False, verbose=True):
    ...
    try:
        ...
        for file in glob.glob("*.npz"):
            ...
            # 使用os.path.normpath标准化路径，避免混合使用/和\
            file_path = os.path.normpath(file_path)
            
            try:
                # 加载文件内容
                results = load_results("", file, verbose=False)
                list_of_experiments += [Experiment(hp_dict=results)]
    ...
    if not list_of_experiments and verbose:
...
def get_experiment(path, name, verbose=False):
    ...
    # 使用os.path.join和normpath标准化路径
    file_path = os.path.normpath(os.path.join(path, name + ".npz"))
    experiment = Experiment(hp_dict=load_results("", file_path, verbose=False))
